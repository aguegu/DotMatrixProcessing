...
public class TetrisDemo extends PApplet
{
	...
	for (int i = 0; i < frame.length; i++)
	    df.add(new DotFont(_dm, frame[i], frameLoaction[i][0],
		    frameLoaction[i][1], FontDirection.HORIZ0NTAL));

	tscore = new TetrisScore(_dm, 2, 0, 0, score);

	tbNext = new TetrisBlock(_dm,
		(int) random(TetrisBlock.getPatternCount()), (int) random(4));
	initBlocks();
